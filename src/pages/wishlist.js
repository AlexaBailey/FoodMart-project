...
useEffect(() => {
  ...
  const receiveInfo = async () => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:8800/favourite/${jwtUser.id}`,{params:{id:jwtUser.id}});
...
const handleSubmit= async (e) => {
    ...
    try {
      e.preventDefault()
      const res = await axios.post(`http://localhost:8800/favourite/${jwtUser.id}`,{params:{id:jwtUser.id},search:search});
      setAllProducts(res.data);
